...
internal class JapScan(context: MangaLoaderContext) :
	PagedMangaParser(context, MangaParserSource.JAPSCAN, 42), Interceptor {
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		try {
			val primaryDoc = fetchDocument(chapterUrl)
			if (isReaderCaptchaPage(primaryDoc)) {
				context.requestBrowserAction(this, chapterUrl)
				return emptyList()
			}
			extractReaderPageUrls(primaryDoc)?.let { pages ->
				return appendReaderPages(pages.urls, pages.queryKey, pages.queryValue)
			}
			for (candidateUrl in buildFallbackReaderUrls(chapter.url)) {
				val doc = fetchDocument(candidateUrl.toAbsoluteUrl(domain))
				if (isReaderCaptchaPage(doc)) {
					context.requestBrowserAction(this, chapterUrl)
					return emptyList()
				}
				extractReaderPageUrls(doc)?.let { pages ->
					return appendReaderPages(pages.urls, pages.queryKey, pages.queryValue)
				}
			}
			return emptyList()
	...
	private fun extractReaderPageUrls(doc: Document): ReaderPages? {
		val readerConfig = parseReaderConfig(doc) ?: return null
		val payloadElement = doc.selectFirst("[data-${readerConfig.datasetKey}]")
			?: readerConfig.tag?.let { doc.getElementsByTag(it).firstOrNull() }
			?: return null
		val rawPayload = extractReaderPayload(payloadElement, readerConfig.datasetKey)
		...
		val payloadJson = decodeReaderPayloadObject(slicedPayload) ?: return null
		val pageUrls = extractPageUrls(payloadJson, readerConfig.pageArrayKey)
		return if (pageUrls.isEmpty()) {
			null
		} else {
			ReaderPages(
				urls = pageUrls,
				queryKey = readerConfig.queryKey,
				queryValue = readerConfig.queryValue,
			)
		}
	}

	private fun extractReaderPayload(payloadElement: Element, datasetKey: String): String {
		val preferred = payloadElement.attr("data-$datasetKey").trim()
		if (preferred.isNotEmpty()) {
			return preferred
		}
		return payloadElement.attributes().asList()
			.asSequence()
			.filter { it.key.startsWith("data-") }
			.map { it.value.trim() }
			.filter { it.length >= MIN_READER_PAYLOAD_LENGTH }
			.maxByOrNull { it.length }
			.orEmpty()
	}

	private fun buildFallbackReaderUrls(chapterUrl: String): List<String> {
		val normalized = chapterUrl.trim()
	...
	private fun isReaderCaptchaPage(doc: Document): Boolean {
		val hasReaderMarkers = doc.selectFirst("d-24a04, [data-a7b4d4]") != null || parseReaderConfig(doc) != null
		if (!hasReaderMarkers) {
			return false
		}
		return doc.select("script").any { script ->
			val data = script.data().ifBlank { script.html() }
			"__captcha" in data && READER_CAPTCHA_NEEDED_REGEX.containsMatchIn(data)
		}
	}

	private fun decodeReaderPayloadObject(rawPayload: String): JSONObject? {
		val decodedPayload = decodeReaderPayload(rawPayload) ?: return null
		runCatching { JSONObject(decodedPayload) }.getOrNull()?.let { return it }
		val jsonStart = decodedPayload.indexOf('{')
		val jsonEnd = decodedPayload.lastIndexOf('}')
		if (jsonStart < 0 || jsonEnd <= jsonStart) {
			return null
		}
		return runCatching {
			JSONObject(decodedPayload.substring(jsonStart, jsonEnd + 1))
		}.getOrNull()
	}

	private fun appendReaderPages(pageUrls: List<String>, queryKey: String, queryValue: String): List<MangaPage> {
		return pageUrls.map { pageUrl ->
			val absolute = pageUrl.toAbsoluteUrl(domain)
			val finalUrl = appendReaderToken(absolute, queryKey, queryValue)
			MangaPage(
	...
	private fun parseReaderConfig(doc: Document): ReaderConfig? {
		for (script in doc.select("script")) {
			val data = script.data().ifBlank { script.html() }
			parseReaderConfigFromEncodedArray(data)?.let { return it }
			parseReaderConfigFromObjectLiteral(data)?.let { return it }
		}
		return null
	}

	private fun parseReaderConfigFromEncodedArray(scriptData: String): ReaderConfig? {
		for (match in READER_CONFIG_ARRAY_REGEX.findAll(scriptData)) {
			val encoded = READER_CONFIG_ARRAY_ITEM_REGEX.findAll(match.groupValues[1])
				.joinToString(separator = "") { it.groupValues[1] }
			if (encoded.isEmpty()) continue
			val decoded = decodeBase64Relaxed(encoded)?.toString(Charsets.UTF_8) ?: continue
			val payload = runCatching { JSONObject(decoded) }.getOrNull() ?: continue
			val datasetKey = payload.optString("d").trim()
			if (datasetKey.isEmpty()) continue
			return ReaderConfig(
				datasetKey = datasetKey,
				offset = payload.optInt("s", 0),
				queryKey = payload.optString("p").trim(),
				queryValue = payload.optString("v").trim(),
				tag = payload.optString("t").trim().ifEmpty { null },
				pageArrayKey = payload.optString("il").trim().ifEmpty { DEFAULT_PAGE_ARRAY_KEY },
			)
		}
		return null
	}

	private fun parseReaderConfigFromObjectLiteral(scriptData: String): ReaderConfig? {
		val rcBlock = READER_CONFIG_BLOCK_REGEX.find(scriptData)?.groupValues?.getOrNull(1)
			?: return null
		...
		val tag = READER_CONFIG_TAG_REGEX.find(rcBlock)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { null }
		val pageArrayKey = READER_CONFIG_PAGE_ARRAY_KEY_REGEX.find(rcBlock)
			?.groupValues
			?.getOrNull(1)
			?.trim()
			?.ifEmpty { null }
			?: DEFAULT_PAGE_ARRAY_KEY
		return ReaderConfig(
			datasetKey = datasetKey,
			offset = offset,
			queryKey = queryKey,
	...
	private fun extractPageUrls(payload: JSONObject, preferredKey: String): List<String> {
		payload.optJSONArray(preferredKey)
			?.let(::extractUrlsFromArray)
			?.distinct()
			?.takeIf { it.size >= MIN_PAGE_CANDIDATE_SIZE }
			?.let { return it }
	...
	override fun intercept(chain: Interceptor.Chain): Response {
		...
		if (!host.endsWith(JAPSCAN_DOMAIN_SUFFIX) || host == domain) {
		...
		val headers = request.headers.newBuilder()
			...
			.set(CF_WORKER_TOKEN_HEADER, CF_WORKER_TOKEN)
			.build()
	...
	private data class ReaderConfig(
		...
		val tag: String?,
		val pageArrayKey: String,
	)

	private data class ReaderPages(
	...
	private companion object {
		private const val CF_WORKER_TOKEN = "a8f3d2e1b9c4f7a2"
		private const val CF_WORKER_TOKEN_HEADER = "CF-Worker-Token"
		private const val DEFAULT_PAGE_ARRAY_KEY = "ce"
		private const val JAPSCAN_DOMAIN_SUFFIX = ".japscan.foo"
		private const val MIN_PAGE_CANDIDATE_SIZE = 2
		private const val MIN_READER_PAYLOAD_LENGTH = 128
		...
		private val READER_CONFIG_ARRAY_REGEX = Regex(
			"""var\s+web\d+\s*=\s*\[(.*?)]\s*;""",
			setOf(RegexOption.DOT_MATCHES_ALL),
		)
		private val READER_CONFIG_ARRAY_ITEM_REGEX = Regex("""["']([^"']+)["']""")
		private val READER_CONFIG_BLOCK_REGEX = Regex(
			"""window(?:\[['"]__rc['"]]|(?:\.__rc))\s*=\s*\{(.*?)\}\s*;""",
			setOf(RegexOption.DOT_MATCHES_ALL),
		)
		private val READER_CONFIG_DATASET_REGEX = Regex("""(?:^|[,{\s])["']?d["']?\s*:\s*["']([^"']+)["']""")
		private val READER_CONFIG_OFFSET_REGEX = Regex("""(?:^|[,{\s])["']?s["']?\s*:\s*(-?\d+)""")
		private val READER_CONFIG_TAG_REGEX = Regex("""(?:^|[,{\s])["']?t["']?\s*:\s*["']([^"']+)["']""")
		private val READER_CONFIG_QUERY_KEY_REGEX = Regex("""(?:^|[,{\s])["']?p["']?\s*:\s*["']([^"']+)["']""")
		private val READER_CONFIG_QUERY_VALUE_REGEX = Regex("""(?:^|[,{\s])["']?v["']?\s*:\s*["']([^"']+)["']""")
		private val READER_CONFIG_PAGE_ARRAY_KEY_REGEX = Regex("""(?:^|[,{\s])["']?il["']?\s*:\s*["']([^"']+)["']""")
		private val READER_CAPTCHA_NEEDED_REGEX = Regex("""needed\s*:\s*true""")
		private val READER_SUBSTITUTION_MAP: Map<Char, Char> = run {
			val from = READER_SUBSTITUTION_FROM
			val to = READER_SUBSTITUTION_TO
		...
		private const val READER_SUBSTITUTION_FROM = "w3rhxFeZQIC7UcHzjfuNL9aq0yGYP8DVRJTo5E41pA62bgtBvOdlWnKMSsmXki"
		private const val READER_SUBSTITUTION_TO = "nDfF8sB5kGMRIPLwo2t6gj4y39lrH0WuNbCUQS1pcAViEmvO7JKqzdxhYTZaeX"
